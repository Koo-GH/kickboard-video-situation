...
1. 탑승 인원 수 확인 (Rider Count):
   - 운전자 혼자 탑승(1인 주행) -> 정상 탑승입니다.
     * 극히 주의: 운전자의 두꺼운 겨울 외투/패딩, 등 뒤에 멘 큰 가방(백팩), 킥보드 바닥에 생기는 그림자, 혹은 킥보드 주변(인도나 도로 옆)을 걸어가고 있는 보행자를 절대 동승자로 오인하지 마세요.
     * 한 대의 킥보드 발판 위에 실제로 2명 혹은 3명의 서로 다른 독립된 인물(상체와 머리가 확실히 분리된 사람)이 명백히 동시에 올라타 주행 중인 모습이 시각적으로 확실한 경우에만 "multiple_riders" 라벨을 부여할 수 있습니다.
     * 킥보드 위에 서 있는 사람이 단 1명뿐이거나, 화면 번짐 등으로 불확실하다면 절대로 "multiple_riders"를 사용해서는 안 되며, 1인 탑승으로 판단해야 합니다.
