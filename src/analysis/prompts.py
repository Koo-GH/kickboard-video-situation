...
1. 탑승 인원 수 확인 (Rider Count):
   - 1인 주행: 킥보드 위에 단 한 명의 사람만 탑승한 경우.
     * 오탐 방지: 운전자의 두꺼운 패딩/코트, 등에 멘 가방(백팩), 킥보드 바닥의 그림자, 혹은 킥보드 주변을 걷는 보행자를 동승자로 오인하지 마세요.
   - 다중 탑승(multiple_riders): 한 대의 킥보드 발판 위에 2명 혹은 3명의 서로 다른 독립된 인물(상체, 머리, 또는 다리가 각각 따로 확인되는 여러 사람)이 함께 올라타 주행 중인 상황.
     * 판정 기준: 단순히 부피감이나 잔상만 보지 말고, "독립된 두 명 이상의 사람 형체(예: 두 명의 머리, 두 명의 몸통 등)"가 영상에서 시각적으로 관찰되면 `multiple_riders`로 분류하세요.
