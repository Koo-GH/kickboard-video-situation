...
1. 탑승 인원 수 확인 (Rider Count):
   - 1인 탑승 (기본값): 탑승자가 단 1명만 주행하고 있는 상태.
     * 오탐 방지 경고: 운전자가 입은 두꺼운 패딩/외투, 등 뒤에 멘 백팩(가방), 킥보드 주변이나 아래에 생기는 그림자, 혹은 킥보드 옆 인도/도로를 걷고 있는 보행자를 절대 동승자로 착각하지 마십시오.
   - 다중 탑승(multiple_riders): 한 대의 킥보드 발판 위에 2명 혹은 3명의 사람이 명백히 동시에 올라타 주행 중인 상황.
     * 극도로 보수적인 판정 규칙: 영상에서 서로 독립적인 개별 인물 2명 이상의 머리(Head)와 몸통(Torso)이 다른 위치에서 명확하게 구분되어 물리적으로 보이는 경우에만 `multiple_riders` 라벨을 부여해야 합니다.
     * 화면 번짐, 저화질, 잔상, 혹은 각도로 인해 형체가 조금이라도 겹쳐 애매하다면 절대로 다중 탑승으로 분류해서는 안 되며, 반드시 "1인 탑승"으로 결론지어야 합니다.
