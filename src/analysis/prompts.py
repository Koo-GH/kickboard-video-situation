...
1. 탑승 인원 수 확인 (Rider Count):
   - 1인 탑승(single_rider): 킥보드에 단 한 명의 운전자만 타고 있는 정상 주행 상태.
     * 판정 기준: 1인 단독 주행 중인 영상이라면 **반드시 "single_rider" 라벨을 기입하십시오.**
     * 오탐 방지 경고: 운전자가 입은 두껍고 폼이 넓은 아우터/패딩, 등 뒤에 멘 큰 가방(백팩), 킥보드 밑바닥에 생기는 어두운 그림자, 킥보드 바로 옆 도로나 인도를 걷고 있는 보행자를 절대 동승자로 오인하여 `multiple_riders`를 부여하면 안 되며, `single_rider`를 사용해야 합니다.
   - 다중 탑승(multiple_riders): 한 대의 킥보드 발판 위에 실제로 2명 혹은 3명의 사람이 동시에 올라타 주행 중인 상황.
     * 극도로 보수적인 감지 규칙: 영상에서 서로 독립적인 개별 인물 2명 이상의 머리(Head)와 얼굴/상체가 다른 위치에서 명확하고 뚜렷하게 따로 분리되어 보이는 경우에만 `multiple_riders` 라벨을 적용할 수 있습니다. 조금이라도 흐릿하거나 겹쳐서 구분이 애매하다면 무조건 "1인 탑승"으로 간주하고 `single_rider` 라벨을 사용해야 합니다.
...
3. 함께 관찰된 **나머지 모든 위험 상황 라벨들은 `secondary_situations` 배열에 빠짐없이 기입하세요.**
   - 예시 A: 혼자 차도에서 헬멧 없이 달린다면 -> primary_situation: "roadway_riding", secondary_situations: ["no_helmet", "single_rider"]
   - 예시 B: 혼자 헬멧 없이 보행자 곁을 차도에서 달린다면 -> primary_situation: "pedestrian_risk", secondary_situations: ["roadway_riding", "no_helmet", "single_rider"]
...
선택 가능한 후보 라벨 목록:
normal_riding, roadway_riding, no_helmet, multiple_riders, single_rider, pedestrian_risk, vehicle_risk, obstacle_risk, intersection_crosswalk, sudden_stop_or_evasion, fall_or_crash, poor_road_condition, sidewalk_riding, wrong_way_or_lane_issue, needs_review
...
근거(evidence)는 최종 결정된 상황(primary_situation 및 secondary_situations)을 직접적으로 증명하는 시각적 사실만 구체적인 한국어 문장 배열로 작성하세요.
  - 주의: "그림자를 오인하지 않았다", "두꺼운 겨울 외투를 입었다", "그림자가 보이지 않는다" 등 판단 기준이나 분석 과정을 설명하는 메타 문장은 절대로 기입하지 마세요. 오직 실제 감지된 위험 상황을 입증하는 직접적인 사실만 포함해야 합니다.
